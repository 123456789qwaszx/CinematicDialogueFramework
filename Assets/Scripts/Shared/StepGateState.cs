...
{
    public List<GateToken> StepGates;  // Resolved gate tokens for all steps in the current node
    ...
    public GateToken? CurrentToken
    {
        get
        {
            if (StepGates == null) return null;
            if (StepIndex < 0 || StepIndex >= StepGates.Count) return null;
            return StepGates[StepIndex];
        }
    }
}
