...
public class DialogueSession
{
    ...
    private DialogueSituationSpec _spec;
    ...
    {
        var (spec, state) = _resolver.ResolveNew(situationKey);
        if (spec == null || state == null) return false;
    ...
    public void Tick()
    {
        ...
        if (_state.NodeCursor < 0 || _state.NodeCursor >= _spec.nodes.Count)
        {
        ...
        // Process the current node's GateTokens
        ...
        // If the node gate is complete, advance to the next node
        if (_state.IsNodeGateCompleted)
        {
            _state.NodeCursor++;

            if (_state.NodeCursor >= _spec.nodes.Count)
            {
                _presenter.PresentSystemMessage("(End of Situation)");
                OnSituationEnded?.Invoke();
                Stop();
            ...
            _resolver.ResolveCurrentNodeGate(_spec, _state);
            EnterNode();
            progressed = true;
        }

        // 'progressed' can be used for logging/debugging
    }

    private void EnterNode()
    {
        var vm = _resolver.BuildNodeViewModel(_spec, _state);
    ...
    {
        var (spec, state) = _resolver.ResolveNew(save.SituationKey);
        if (spec == null || state == null) return false;
